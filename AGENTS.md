...
- Endpoints: `/status` (job health + snapshot freshness — the "Data status" surface for judges), `/universe?lane=meme|coins|bstocks`, `/tokens/:addr` + batch `/tokens?addresses=`, `/klines/:addr`, `/security/:addr`, `/holders/:addr`, `/socials/:addr`, `/eligibility/:addr` + batch `/eligibility?addresses=`, `/pools`, `/pools/top`, `/pools/:addr`, `/pools/:addr/range`, `/venus/:owner`, `/diag/latency`. `npm run loadtest` — hit 300 rps, 0 errors, p95 ~37ms.
...
## Pool lane / yield (`src/jobs/pancakePools.ts`, `/pools/top`)

The Yield Optimisation surface. **BSC + PancakeSwap V3 only** — Infinity, v2 and StableSwap are out of scope; `protocol` is on the record so adding a lane later is additive.

Everything comes from PancakeSwap's own keyless explorer API, the same service their web app reads, which is what makes matching their UI's APR a property of the architecture rather than of a formula kept in sync by hand. Measured 2026-08-14: median 73ms, **no rate limit reachable** (60 parallel requests, all 200, no `x-ratelimit-*`, no 429), `s-maxage=100` so 60s is the useful polling floor. One live cycle: **2.0s, 500 pools, 10 pages, 43 farmed**.

- **APR is two things and they are never conflated.** `lpFeeApr24h`/`lpFeeApr7d` = the explorer's `apr24h`/`apr7d` × 100, taken verbatim. `cakeFarmApr` = MasterchefV3 `0x556B9306565093C855AEA9AE92A594704c2Cd59e`, `cakePerSecond × allocPoint/totalAllocPoint × 31_536_000 × cakePrice / tvl` (selectors `0xc4f6a8ce`, `0x17caf6f1`, `0x0743384d`, `0x1526fe27`; `cakePerSecond` is raw / 1e12 / 1e18). `combinedApr` is their sum = the UI's APR column. Verified to two decimals against the UI on quq/USDT (14.97), USDT/WBNB 0.01% (3.19), USDC/WBNB 0.01% (0.63), and re-verified end to end against a raw `eth_call` recomputation.
- **Never derive fee APR from `feeUSD24h`.** That field is gross of the ~33% protocol fee; annualizing it overstates the LP's APR ~1.5x and does not match the UI. This *was* the bug (`aprPct`), and there is a test pinning it shut.
- **`0` and `null` mean different things and this is load-bearing.** `cakeFarmApr: 0` = the farming list does not carry this pool, so it earns no CAKE. `null` = the farm read failed. `combinedApr` is `null` unless both components are known — a sum missing a term reads as a smaller yield rather than as a gap, and `aprSources` names what is actually in it. If MasterChefV3 is unreadable the whole lane keeps `null`, never `0`.
- **Scope decision — do not re-propose (2026-08-14):** third-party incentives are **out**. Measured: Merkl has exactly 2 live Pancake opportunities on BSC and both report `apr: 0`; Incentra has **zero** BSC campaigns and never has (399 campaigns, all other chains, all ENDED/failed); Ethena/Falcon points are hardcoded in the frontend (`FarmStatusDisplay/config.ts`, 4 BSC pools) with no APR and no API. So `lpFee + cakeFarm` *is* the UI number on this chain.
- **The plane sets no thresholds.** TVL floors, fee tiers, APR bands and volume/TVL screens are all `/pools/top` query params. The lane is capped at 500 and ingested in TVL order — a ranking, not a verdict — and `meta.cap`/`meta.ingestOrder` say so. Sorting the raw lane by APR puts a $13 pool at 11,556% on top; that is the caller's to filter, and the argument for the `tier` labels.
- **`tier` labels the tokens, not the pool's numbers** (`src/query/poolTier.ts`). TVL, volume and APR are all things a permissionless pool can manufacture — the numbers *are* the thing being faked — so classification looks at provenance instead. `core` = both tokens curated by somebody (the eligible-token allowlist, Binance Alpha, or PancakeSwap Extended). `degen` = both tokens accounted for and at least one off Four.Meme or Flap. `tokenOrigin` carries the evidence so a label can be argued with. Measured on a live lane 2026-08-14: **301 core, 46 degen, 153 unclassified** — and the entire wash-trade cluster (GULD/SWITCH/ELMT et al., TVL reported as $6e16) lands in `unclassified` with both origins `unknown`, which is the whole point.
- **Launchpad origin is read from the chain and cached forever (`origins:launchpad`) — do not go back to the ranking lanes.** It was built from `universe:meme`/`universe:flap` first and that was wrong: those lanes are *leaderboards*. Measured 2026-08-14 — of the 113 tokens then ranked, only **3** had any V3 pool and only **1** cleared the lane's TVL cutoff (which is just **$344**, so the cap was never the constraint); meanwhile every graduate with a real V3 pool — TUT, Broccoli, AKE, BR, mubarak, TST — had dropped out of the top 100 and came back `core` off the allowlist. Worse, AKE/WBNB flipped between `degen` and `core` across two runs depending on whether its token was trending. A label that moves with a ranking is not provenance. Fixing it took `degen` from **1 pool to 46**.
- **That is affordable because the answer is permanent.** A launchpad mints new contracts, it never adopts an existing one, so `none` is as final as `fourmeme`; `resolveLaunchpadOrigins` (in `eligibility.ts`, reusing the gate's own two reads so there is one definition of the question) is asked once per token, ever. Measured: 376 distinct tokens across a full 500-pool lane, 12.5s cold. Capped at **150 tokens per cycle** so a cold start converges over three cycles at ≤2.5s each rather than in one burst — the whole cache is written once at the end of a pass, so a pass the job timeout kills saves nothing, and on a host with slower chain reads an uncapped pass might never finish. Warm cycles are ~1.3s and resolve nothing. A chunk no endpoint could serve is **omitted, never written as `none`** — this cache is forever, and an outage written into it would be too.
- Curated origins stay store-only and cost three reads a cycle: the frozen allowlist file, `universe:coins`, and PancakeSwap Extended (`tokens.pancakeswap.finance`, 972 BSC entries, cached 24h in `pancake:tokenlist`).
- One deliberate limit remains, so it does not read later as a bug: the tier says nothing about CAKE emissions, because `farm.allocPoint` answers a different question and is already on the record.
- **Labels are sticky.** Provenance does not change, so an `unclassified` verdict on a previously labelled pool is nearly always a source being unavailable rather than news; a different real label still replaces the old one. With the frozen allowlist unreadable the pass is skipped outright, the same way the eligibility gate refuses to answer without it.
- `GET /pools/:address` is read-through in three steps — lane, cached `pool:<addr>`, live explorer read cached on the way out — and `meta.source` says which answered, because only a lane row has been through farm pricing and classification.

### Range economics (`src/query/poolRange.ts`, `/pools/:address/range`)

`/pools/top` answers *which pool* with the pool's APR. `/pools/:addr/range?lower=&upper=&capital=` answers *which range*, which on a concentrated AMM is a different number and the one an LP agent acts on. Measured live on USDT/WBNB 0.01% against a pool APR of 9.82%: ±50% → 2.62% (0.3x), ±10% → 12.75% (1.3x), ±2% → 62.42% (6.4x), ±0.5% → 244.39% (25.2x).

- **No tick scan is needed, and that was the surprise.** Fees go to in-range liquidity, so a position's share is `L / (L_active + L)` — `L` follows from the capital and the bounds, `L_active` is the pool's own `liquidity` field. The tick array (measured: 20,323 ticks over 21 pages, 2.7s for the deepest pool) would only be needed to model the price *moving* across ticks, a different question. Do not add it for this.
- **APR is scaled off `lpFeeApr24h`, never re-derived from fee totals** — same reason as the pool lane, and the identity `positionApr = poolApr × concentrationMultiplier` (before self-dilution) has a test pinning it, which keeps the estimate anchored to the figure verified against PancakeSwap's UI.
- **Exact vs assumed is kept apart in the payload.** Exact: `position.liquidity`, `amount0/1`, `feeSharePct`, `inRange`. Assumed: that volume repeats and the price stays in range — `assumptions` says so on every response, and `unavailable` names any missing input so a `null` is always attributable. Impermanent loss is deliberately not modelled: it is a function of where the price ends up, and an invented number would be indistinguishable from the real ones.
- Share is `L/(L_active + L)`, **not** the small-position shortcut `L/L_active`: a position large next to its pool dilutes itself, and at $100M into a $100M pool the difference is the whole answer.
- Tick spacing per fee tier, read off the pool contracts 2026-08-14: `100→1, 500→10, 2500→50, 10000→200`. Bounds snap **outward**, so the placeable range always contains the one asked for.
- Inputs (pool state + APR + both token USD prices) cache together as `pool:range:<addr>` for 60s. Twenty candidate ranges over one pool are twenty questions about one set of facts and cost one fetch — measured 4ms cold, <1ms warm.
- `String(2 ** 96)` yields exponential notation and is rejected by `parseBigintString`. Write Q96 out in full.
- Lane fails **open**: a complete paging pass replaces the lane, a broken one is backfilled from what was stored, and a cycle that reads *nothing* throws without republishing — restamping would age stale rows into looking freshly read. Each row carries its own `asOf` alongside the snapshot's.
- Storage: the lane is **one** key (`universe:pools`), not 500. Per-pool `pool:<addr>` keys are written only for the operator-tracked seed set that `/pools` serves; those are also the only rows carrying chain state, since the list endpoint does not return `liquidity`/`sqrtPrice`/`tick`. Seed pools merge ahead of the cap so they can never be evicted.
- `npm run smoke` covers all four read paths; `node --import tsx scripts/lane-check.ts` runs one real cycle and queries it.
