...
export function featureKey(pool: string, interval: FeatureInterval, currency: "usd" | "token" = "usd", tokenAddress?: string): string {
  return `trading:features:v1:${pool.toLowerCase()}:${interval}:${currency}${tokenAddress ? `:${tokenAddress.toLowerCase()}` : ""}`;
}
...
export function calculateFeatures(input: FeatureInput, now: number): FeatureSnapshot {
  ...
  for (const bar of input.candles) {
    if (!Number.isSafeInteger(bar.timestamp) || bar.timestamp <= 0 || bar.timestamp % step !== 0) { invalidBars++; continue; }
    ...
    if (!validPrice(bar)) { invalidBars++; conflicts.add(bar.timestamp); continue; }
  ...
  const reasonFor = (required: number): UnavailableReason | null => {
    ...
    if (invalidBars) return "invalid_bar";
    if (conflicts.size) return "conflicting_revision";
  ...
  const ema = (period: number): number => {
    const prices = bars.slice(-FEATURE_HISTORY).map((b) => b.close);
  ...
  const atr = (): number => {
    const window = bars.slice(-FEATURE_HISTORY);
  ...
  return {
    version: FEATURE_VERSION, seriesId, snapshotId: hash({ version: FEATURE_VERSION, input: retainedInput, cutoff }), input: retainedInput,
    ...
    parameters: { historyBars: 120, rocPeriod: 10, rvolBaseline: 20, emaPeriods: [12, 26], atrPeriod: 14,
      ...
      schedulingGraceMs: SCHEDULING_GRACE_MS, bucketConvention: "UTC-open-ms" },
    metrics: {
      roc10Pct: metric(11, "percent", () => (latest!.close / bars.at(-11)!.close - 1) * 100),
      ema12: metric(120, priceUnit, () => ema(12)), ema26: metric(120, priceUnit, () => ema(26)),
      emaSpreadPct: metric(120, "percent", () => (ema(12) / ema(26) - 1) * 100),
      atr14: metric(120, priceUnit, atr), atrPct: metric(120, "percent", () => atr() / latest!.close * 100),
...
export async function readTradingFeatures(store: SnapshotStore, pool: string, interval: FeatureInterval, now = Date.now(), currency: "usd" | "token" = "usd", tokenAddress?: string) {
  const record = await store.get<FeatureSnapshot>(featureKey(pool, interval, currency, tokenAddress));
  if (!record || record.data.version !== FEATURE_VERSION) return null;
