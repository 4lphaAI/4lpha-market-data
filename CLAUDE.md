...
- **It is much slower from Railway than locally**: ~19.5s per cycle in production against ~1.7s from a local machine, with all three chunks served and no failures. That is `publicnode` latency from the SE-Asia region, not errors — but it spends most of the 30s timeout, and `publicnode` is the only public endpoint serving recent log ranges. Setting `BSC_RPC_URL` to a paid endpoint removes the exposure; the code already tries configured endpoints first.
